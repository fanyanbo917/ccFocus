...
export class strategyCollision extends Strategy {

    static instance: strategyCollision
    static get() {
        if (!strategyCollision.instance) {
            strategyCollision.instance = new strategyCollision()
        }
        return strategyCollision.instance
    }
    private constructor() { 
        super()
        log.info('strategyCollision constructor')
    }
    
    /**
     * @description: 向上移动，寻找最佳落焦元素
     * 策略：遍历所有可落焦元素，当前元素和非上边元素跳过，下边元素分两种情况：
     * 1. 有重叠部分，只计算Y轴距离, 优先级高，如果有重叠元素，不再查找无重叠部分元素，只返回重叠元素中的最优元素
    ...
    getUpTarget(focusableEles: HTMLCollectionOf<Element>, curFocusEle: Element): null | Element {
        ...
        let collisionMinDist = 99999, nocollisionMinDist = 99999, isFindCollision = false, _tempTarget = null, len = focusableEles.length
        ...
        for (let i = 0; i < len; i++) {
            ...
            if (_ele !== curFocusEle) {
                ...
                if (_eleTop < _curFocusEleTop && (leftCollision || rightCollision)) {
                    ...
                    if (_dist < collisionMinDist) {
                        collisionMinDist = _dist
                        _tempTarget = _ele
                    }
                    isFindCollision = true
                    console.log('检测有重叠区域，只计算Y轴距离', _dist, collisionMinDist)
                } else if ((_eleTop < _curFocusEleTop) && !isFindCollision) {
                    ...
                    if (_dist < nocollisionMinDist) {
                        nocollisionMinDist = _dist
                        _tempTarget = _ele
                    }
                    console.log('检测无重叠区域，计算距离', _dist, nocollisionMinDist)
                } else {
     ...
     * 1. 有重叠部分，只计算X轴距离，优先级高，如果有重叠元素，不再查找无重叠部分元素，只返回重叠元素中的最优元素
        ...
        for (let i = 0; i < len; i++) {
            ...
            if (_ele !== curFocusEle) {
                ...
                if (_eleLeft < _curFocusEleLeft && (upCollision || downCollision)) {
                    ...
                    isFindCollision = true
                    console.log('检测有重叠区域，只计算X轴距离', _dist, collisionMinDist)
                } else if ((_eleLeft < _curFocusEleLeft) && !isFindCollision) {
